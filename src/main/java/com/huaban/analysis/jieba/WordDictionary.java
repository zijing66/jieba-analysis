...
public class WordDictionary {
    ...
    public void init(Path configFile) {
        ...
        synchronized (WordDictionary.class) {
            ...
            try {
                stream =
                    Files.newDirectoryStream(configFile, String.format(Locale.getDefault(), "*%s", USER_DICT_SUFFIX));
                ...
                // e.printStackTrace();
                System.err
                    .println(String.format(Locale.getDefault(), "%s: load user dict failure!", configFile.toString()));
            }
        }
    }
    ...
    public void loadUserDict(String userDictPath, Charset charset) {
        ...
        try {
            ...
            System.out.println(String
                .format(Locale.getDefault(), "user dict %s load finished, tot words:%d, time elapsed:%dms",
                    userDictPath, count, System.currentTimeMillis() - s));
            br.close();
        } catch (IOException e) {
